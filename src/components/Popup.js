...
export default function Popup({works, clicked, openPopup}) {
  ...
  return (
    <div className='fixed top-0 left-0 bg-dawn-pink-50 w-screen sm:w-full h-screen sm:h-full z-20'>
      <div className='flex items-center justify-center h-full'>
        ...
        <div className='hidden sm:block absolute top-[50vh] right-0 cursor-pointer group' onClick={()=>handleChangeImage(1)}>
          <img className='w-8 m-6 animate-wiggle hover:animate-slide duration-300 z-20 ' src={arrowR} />
        </div>
        <div className='hidden sm:block absolute top-[50vh] left-0 cursor-pointer group' onClick={()=>handleChangeImage(-1)}>
          <img className='w-8 m-6 animate-wiggle hover:animate-slide duration-300 z-20  ' src={arrowL} />
        </div>
      </div>
    </div>
  )
